        ...
        <el-menu-item class="more-class" v-if="artClassList.length > majorLength">
            ...
            <div class="more-panel" v-show="panel" @mouseenter="showPanel(true)" @mouseleave="showPanel(false)">
                <div v-for="(lineNum, lineIndex) in Math.ceil((artClassList.length - majorLength) / buttonNum)" :key="lineIndex">
                    <el-button type="primary" v-for="(artClass, i) in artClassList.slice(majorLength + lineIndex * buttonNum, majorLength + lineIndex * buttonNum + buttonNum)" :key="i"
                               v-on:click="changeCurIndex(majorLength + lineIndex * buttonNum + i)">
                        {{artClass}}
                    </el-button>
                </div>
            </div>
...
<script>
     ...
     * 传入数据参考:
     *  leftMenu: {
            curIndex: 0,            // 指示当前索引被选中
            majorLength: 8,         // 一级类别的数量
            tip: '更多',             // 打开二级类别面板的按钮的提示信息
