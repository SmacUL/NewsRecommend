...
class CommentDao:
    ...
    def is_com_exist(self, com_spider):
        ...
        """
        try:
            search_sql = "select count(*) from Comments where com_spider = '%s'" % com_spider
    ...
    def search_com_id_by_spider(self, com_spider):
        ...
        """
        try:
            search_sql = "select com_id from Comments where com_spider = '%s'" % com_spider
            self.__base.execute_sql(search_sql)
            result = self.__base.get_result_one()
    ...
    def check_com_cus_relationship(self, art_id, com_id, cus_id):
        try:
            search_sql = "select count(*) from Comments " \
                         "where com_article_id=%d and com_id=%d and com_customer_id=%d" % (art_id, com_id, cus_id)
            self.__base.execute_sql(search_sql)
            result = self.__base.get_result_one()
            if result[0] == 0:
                logging.info("check_com_cus_relationship 评论关系 新闻 art_id=%s 用户 cus_id=%s 数据库查询 不存在" % (art_id, cus_id))
                return False
            else:
                logging.info("check_com_cus_relationship 评论关系 新闻 art_id=%s 用户 cus_id=%s 数据库查询 存在" % (art_id, cus_id))
                return True
        except:
            logging.exception("check_com_cus_relationship 评论关系 新闻 art_id=%s 用户 cus_id=%s 数据库查询 错误" % (art_id, cus_id))
            raise
